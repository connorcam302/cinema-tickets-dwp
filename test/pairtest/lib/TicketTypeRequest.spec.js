  ...
  describe('Requesting Tickets', () => {
    describe('Invalid Request', () => {
      ...
      it('should throw an error if ticket amount is not an integer', () => {
        ...
        invalidTicketAmounts.forEach((ticketAmount) =>
          expect(() => new TicketTypeRequest('ADULT', ticketAmount)).to.throw(TypeError),
        )
      })
    })
    ...
    it('should not allow modification or adding new properties', () => {
      const request = new TicketTypeRequest('ADULT', 2)

      // Trying to overwrite private fields won't work (private fields inaccessible)
      expect(() => {
        request.type = 'CHILD'
      }).to.throw(TypeError)
      expect(() => {
        request.noOfTickets = 999
      }).to.throw(TypeError)

      // Existing getters should still return original values
